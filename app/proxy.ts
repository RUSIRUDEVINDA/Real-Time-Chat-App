...
export const proxy = async (req: NextRequest) => {
  ...
  // Parse connected array (Redis returns it as string if it's a single value or array)
  let connectedTokens: string[] = []
  if (meta.connected) {
    connectedTokens = Array.isArray(meta.connected)
      ? meta.connected
  ...
  await redis.hset(`meta:${roomId}`, {
    connected: JSON.stringify([...connectedTokens, token]),
  })
